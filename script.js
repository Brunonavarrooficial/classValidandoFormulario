class ValidaFormulario {
  ...
  handleSubmit(e) {
    e.preventDefault();
    const todosCamposValidos = this.camposValidos();
    const senhasSaoValidas = this.senhasValidas();

    if (todosCamposValidos && senhasSaoValidas) {
  ...
  senhasValidas() {
    ...
    if (senha.value.length < 6 || senha.value.length > 12) {
      valid = false;
      this.criaErro(senha, 'Senha ptrecisa estar entre 6 e 12 caracteres');
    }
